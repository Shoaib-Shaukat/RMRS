...
export class NavContentComponent implements OnInit, AfterViewInit {
  ...
  constructor(private menuService: MenuService, public nav: NavigationItem, private zone: NgZone, private location: Location, public navTwo: NavigationItem, public restaurantService: RestaurantService) {
    ...
    this.restaurantService.sendFlagFurhter.subscribe(
      x => {
        if (localStorage.getItem("isStaff") == "true") {
          ...
          this.navigation = this.navTwo.getTwo();
          this.Permission = JSON.parse(localStorage.getItem("permissionArr"));
          for (let i = 0; i < this.Permission.length; i++) {
            for (let j = 0; j < this.navigation.length; j++) {
              if (this.navigation[j].title == this.Permission[i]) {
                this.newNavigation.push(this.navigation[j]);
              }
            }
          }
          this.navigation = this.newNavigation;
        }
        else {
          if (x) {
            this.navigation = this.navTwo.getTwo();
